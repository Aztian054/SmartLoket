...
@section('title', 'Alih Media BT — Detail Berkas')
...
@section('content')
    ...
    @include('partials.stage_actions', ['stage' => 'alih_media_btel', 'stageLabel' => 'Alih Media BT'])
    @include('partials.bidang_table')

    @if($isActive && $mine)
        <div class="card card-custom mb-3">
            <div class="card-header bg-white py-3">
                <h6 class="mb-0 fw-bold text-dark"><i class="bi bi-file-earmark-diff-fill text-primary me-2"></i>Lembar Kerja Alih Media BT</h6>
            </div>
            <div class="card-body">
                <form action="{{ route('alih_media_btel.simpan', $tiket->id) }}" method="POST">
                    @csrf
                    <div class="row g-3">
                        <div class="col-md-4">
                            <label class="form-label">Scan Buku Tanah</label>
                            <select name="status_scan_buku_tanah" class="form-select">
                                @foreach(['belum', 'sudah', 'kualitas_buruk'] as $opt)
                                    <option value="{{ $opt }}" {{ ($lembar->status_scan_buku_tanah ?? 'belum') === $opt ? 'selected' : '' }}>{{ ucwords(str_replace('_', ' ', $opt)) }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4">
                    ...
                    <div class="mt-3">
                        <button class="btn btn-gold"><i class="bi bi-save me-1"></i>Simpan Progres Alih Media BT</button>
                    </div>
                </form>
            </div>
        </div>
    @elseif($isActive && !$mine)
        <div class="alert alert-warning"><i class="bi bi-person-lock me-1"></i>Berkas ini sedang diproses akun lain pada tahap Alih Media BT.</div>
    @endif
